...
const onLine = (x1, y1, x2, y2, x, y, maxDistance=1) => {
  ...
  return offset < maxDistance ? "inside": null;
}
...
const adjustElementCoordinates = element => {
  ...
  if (type==="rectangle") {
    ...
    // when draw a line from left to right; or draw vertically from top to bottom
...
// history: [[{element1}, {element2}...], [{element2},{}...], [{},{},{}]]
const useHistory = initialState => {
  const [index, setIndex] = useState(0);
  const [history, setHistory] = useState([initialState]);

  // action: can be a function ((previousState)=>newState) or a new state (a state is a list of elements in this case)
  const setState = (action, overwrite=false) => {
    ...
    if (overwrite) {
      ...
      // discard the states later than the current index
...
const getSvgPathFromStroke = stroke => {
  ...
  const d = stroke.reduce(
    // accumulator, current value, index
    (acc, [x0, y0], i, arr) => {
...
const adjustmentRequired = type => ['line', 'rectangular'].includes(type);
...
function App() {
  ...
  const handleMouseDown = (event) => {
    ...
    if (tool === "selection") {
      ...
      if (element) {
        const beforeMoveX = clientX;
        const beforeMoveY = clientY;
        setSelectedElement({...element, beforeMoveX, beforeMoveY});
        
        setElements(prevState => prevState); // make a snapshot in history
    ...
    // when mouse down on "line" or "rectangle" mode
    else {
  ...
  const handleMouseMove = (event) => {
    ...
    if (action === 'drawing') {
      ...
      const {id, x1, y1, x2, y2, beforeMoveX, beforeMoveY, type} = selectedElement;
      const offsetX = clientX - beforeMoveX 
      const offsetY = clientY - beforeMoveY
      if (selectedElement.type ==="pencil") {
        const newPoints = selectedElement.points.map(point => ({x: point.x+offsetX, y:point.y+offsetY}))
        const elementsCopy = [...elements];
        elementsCopy[id] = {
          ...elementsCopy[id],
          points: newPoints
